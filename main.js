...
ipcMain.handle('select-file', async () => {
  const { canceled, filePaths } = await dialog.showOpenDialog(mainWindow, {
    properties: ['openFile'],
    filters: [{ name: 'Documents', extensions: ['pdf','csv','txt','docx','xlsx','xls'] }]
  });
...
/** Collapse very long unchanged runs so the diff is scannable. */
...
// Compare files: { diff, meta } — diff is an array of parts for the UI
ipcMain.handle('compare-files', async (event, file1, file2) => {
  try {
    ...
    const excelMeta =
      (ext1 === '.xlsx' || ext1 === '.xls') && (ext2 === '.xlsx' || ext2 === '.xls')
        ? buildExcelSheetMeta(file1, file2)
        : null;
...
async function extractText(filepath) {
  ...
  } else if (ext === '.xlsx' || ext === '.xls') { // <--- ADD THIS BLOCK
    return await extractExcelText(filepath);
  } else {
    // For text files, use streaming for large files
...
async function extractPdfText(filepath) {
  try {
    ...
    // STEP 1: Fix broken words caused by small columns (e.g., Numbe\nr -> Number)
    // This looks for a newline that happens in the middle of a word
    text = text.replace(/([a-zA-Z])\n([a-z][a-z]+)/g, '$1$2');

    // STEP 2: Normalize line endings and remove weird invisible characters
    text = text.replace(/\r/g, ''); 

    // STEP 3: Collapse multiple spaces into one
    text = text.replace(/[ ]+/g, ' ');

    // STEP 4: Remove empty lines that make the "Diff" look messy
    text = text.split('\n')
               .map(line => line.trim())
...
async function extractCsvText(filepath) {
  try {
    ...
    // Parse CSV and preserve structure
...
async function extractExcelText(filepath) {
  try {
    ...
    workbook.SheetNames.forEach(sheetName => {
      ...
      // Convert the sheet to a CSV-style text format
 ...
 // For large CSVs, include summary info
 if (fileSize > LARGE_FILE_THRESHOLD) {
...
async function extractLargeTextFile(filepath) {
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      text += line + '\n';
      lineCount++;
      // Limit to first 100,000 lines for diff performance
      if (lineCount >= 100000) {
        rl.close();
      }
    });
...
async function extractDocxText(filepath) {
  try {
    ...
    // We apply the same cleaning logic as the PDF to keep the diff clean
